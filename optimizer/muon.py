...
def get_params_for_muon(model) -> List[nn.Parameter]:
  """
...
class Muon_AdamW(ChainedOptimizer):
  ...
  def __init__(self, model, lr=0.0005, weight_decay=0.0, muon_args={}, adamw_args={}, verbose=False):
    ...
    callback = None
    if verbose:
      callback = lambda p, spec_idx: print(f"Adding param {p.shape} to optimizer{spec_idx} {str(specs[spec_idx].class_type)}")
